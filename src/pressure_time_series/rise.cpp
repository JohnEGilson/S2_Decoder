...
void Rise_Data::Decode( std::vector<uint8_t> data) {
	...
	nbyte = ((data[1] & 0x0F) << 8) + data[2];
        int subseg = data[2] & 0x0F;
	uint32_t secs = *reinterpret_cast<uint32_t *>(&data[3]);
	secs = __builtin_bswap32(secs);
	start_time = ptime(date(2000,1,1),seconds(secs));

    for( unsigned int i = 7; i < nbyte-1; i+=3) {
        s.index = i;
...
void Rise_Data::sort() {
    if (Scan.size() > 0) {
        ...
        //Scan.erase(std::unique(Scan.begin(),Scan.end()),Scan.end()); // 2025/08/12 BG rise scans with same pressure are occasionally transmitted (phase 9)
    }
}
